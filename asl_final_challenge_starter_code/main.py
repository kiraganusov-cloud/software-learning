...
# correct validation set predications
correct_counts_validate = 0
for true, pred in zip(y_validate, y_pred_validate):
    if true == pred:
        correct_counts_validate += 1

# ADDED: turn a class number into its letter
def class_to_letter(class_id):
...
for class_id in sorted(total_counts.keys()):
    ...
    print(f"Accuracy for class {class_id} (letter: {class_to_letter(class_id)}): {accuracy:3.0f}%")
print(f"----------")

overall_validation_accuracy = correct_counts_validate / len(y_validate)*100  
print(f"Overall Validation Accuracy: {overall_validation_accuracy:3.1f}%")

overall_accuracy = overall_correct / len(y_test)*100
print(f"Overall Testing Accuracy: {overall_accuracy:3.1f}%")
...
print("The model most often misidefentifies letter B, D, and K")
